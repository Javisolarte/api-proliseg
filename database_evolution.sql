...
CREATE TABLE IF NOT EXISTS public.calendario_eventos (
  ...
  usuario_id integer NOT NULL, -- Dueño del calendario
  titulo character varying NOT NULL,
  descripcion text,
  fecha_inicio timestamp without time zone NOT NULL,
  fecha_fin timestamp without time zone NOT NULL,
  todo_el_dia boolean DEFAULT false, -- Para eventos que duran todo el día
  ubicacion character varying,
  color character varying DEFAULT '#3788d8', -- Para pintar el evento en el front-end
  created_at timestamp without time zone DEFAULT now(),
  updated_at timestamp without time zone DEFAULT now(),
  CONSTRAINT calendario_eventos_pkey PRIMARY KEY (id),
  CONSTRAINT fk_calendario_usuario FOREIGN KEY (usuario_id) REFERENCES public.usuarios_externos(id) ON DELETE CASCADE
);

-- 2. Tabla para programar las notificaciones de esos eventos
CREATE TABLE IF NOT EXISTS public.calendario_recordatorios (
  id integer NOT NULL GENERATED BY DEFAULT AS IDENTITY,
  evento_id integer NOT NULL,
  fecha_programada timestamp without time zone NOT NULL, -- Fecha y hora EXACTA en que debe saltar la alerta
  tipo_notificacion character varying DEFAULT 'sistema' CHECK (tipo_notificacion::text = ANY (ARRAY['correo'::text, 'push'::text, 'sistema'::text])),
  enviado boolean DEFAULT false, -- Marca si ya se creó la notificación en la tabla principal
  created_at timestamp without time zone DEFAULT now(),
  CONSTRAINT calendario_recordatorios_pkey PRIMARY KEY (id),
  CONSTRAINT fk_recordatorio_evento FOREIGN KEY (evento_id) REFERENCES public.calendario_eventos(id) ON DELETE CASCADE
);
