...
CREATE TABLE public.conceptos_turno (
  id integer NOT NULL DEFAULT nextval('conceptos_turno_id_seq'::regclass),
  ...
  created_at timestamp without time zone DEFAULT now(),
  updated_at timestamp without time zone DEFAULT now(),
  CONSTRAINT conceptos_turno_pkey PRIMARY KEY (id)
);
...
CREATE TABLE public.nomina_deducciones (
  ...
  aplica_a character varying NOT NULL,
  activo boolean DEFAULT true,
  created_at timestamp without time zone DEFAULT now(),
  creado_por integer,
...
CREATE TABLE public.nomina_empleado (
  ...
  created_at timestamp without time zone DEFAULT now(),
  provision_prima numeric DEFAULT 0,
  provision_cesantias numeric DEFAULT 0,
  provision_intereses_cesantias numeric DEFAULT 0,
  provision_vacaciones numeric DEFAULT 0,
  provision_pension_empleador numeric DEFAULT 0,
  costo_total_empresa numeric DEFAULT 0,
  ...
  ajuste_salarial numeric DEFAULT 0,
...
CREATE TABLE public.nomina_festivos_colombia (
  id integer NOT NULL DEFAULT nextval('nomina_festivos_colombia_id_seq'::regclass),
  fecha date NOT NULL UNIQUE,
  nombre character varying NOT NULL,
  anio integer NOT NULL,
  tipo character varying DEFAULT 'fijo'::character varying,
  CONSTRAINT nomina_festivos_colombia_pkey PRIMARY KEY (id)
);
...
CREATE TABLE public.puestos_trabajo (
  ...
  salario_fijo numeric DEFAULT 0,
  es_estatal boolean DEFAULT false,
  es_monitoreo boolean DEFAULT false,
...
CREATE TABLE public.turnos (
  ...
  CONSTRAINT turnos_concepto_id_fkey FOREIGN KEY (concepto_id) REFERENCES public.conceptos_turno(id)
);
...
CREATE TABLE public.vacaciones (
  id integer NOT NULL DEFAULT nextval('vacaciones_id_seq'::regclass),
  empleado_id integer NOT NULL,
  fecha_inicio date NOT NULL,
  fecha_fin date NOT NULL,
  fecha_retorno date NOT NULL,
  estado character varying DEFAULT 'pendiente'::character varying CHECK (estado::text = ANY (ARRAY['pendiente'::character varying, 'disfrutada'::character varying, 'cancelada'::character varying, 'pagada'::character varying]::text[])),
  ...
  CONSTRAINT vacaciones_empleado_id_fkey FOREIGN KEY (empleado_id) REFERENCES public.empleados(id),
  CONSTRAINT vacaciones_creado_por_fkey FOREIGN KEY (creado_por) REFERENCES public.usuarios_externos(id)
);
...
CREATE TABLE public.visitas_tecnicas_puesto (
  ...
  fecha_llegada timestamp with time zone DEFAULT now(),
  ...
  documento_generado_id integer,
  estado character varying DEFAULT 'programada'::character varying CHECK (estado::text = ANY (ARRAY['programada'::text, 'en_proceso'::text, 'en_curso'::text, 'completada'::text, 'realizada'::text, 'incumplida'::text, 'cancelada'::text, 'pendiente_no_realizada'::text])),
  asignado_a integer,
